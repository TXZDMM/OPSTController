...
class MonitorThread(threading.Thread):
    ...
    def _handle_change(self, ext, mismatches):
        ...
        # 批量篡改：同一ProgId改多个扩展名 → 强终止进程 + 锁定注册表 + 持续模式
        if is_batch and cur_progid:
            self._persistent_mode = True
            self.persistent_tracker.force_persistent(ext)
            level = self.persistent_tracker.get_lock_level(ext)
            self.log_callback(f"⚠ 批量篡改: {cur_progid} 正在篡改多个扩展名，保护强度{level}：强终止进程 + 锁定注册表", "warn")
            log_event("ALL", "批量篡改", "检测到", f"ProgId={cur_progid}, 保护强度{level}")
            # 强终止进程（不是挂起）
